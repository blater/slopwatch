...
func gradedRustPointerEffects(body, source []token, fields map[string]gradedRustFlow, prefix string) (destroyed, moved bool) {
	return gradedRustPointerEffectsWithContracts(body, source, fields, prefix, gradedRustPointerContracts(source))
}
func gradedRustUnitPointerEffects(body []token, u unit, fields map[string]gradedRustFlow, prefix string) (destroyed, moved bool) {
	return gradedRustPointerEffectsWithContracts(body, u.tokens, fields, prefix, rustUnitPointerContracts(u))
}
func gradedRustPointerEffectsWithContracts(body, source []token, fields map[string]gradedRustFlow, prefix string, contracts map[string]bool) (destroyed, moved bool) {
	...
	for i, t := range body {
		if t.text == "return" && flow.unconditional(body, i) {
	...
	for i, t := range body {
		destroyedNow, movedNow := gradedRustPointerToken(body, source, fields, contracts, declarations, calls, locals, &scopes, i, t, &flow)
		destroyed, moved = destroyed || destroyedNow, moved || movedNow
	}
	return
}

func gradedRustPointerToken(body, source []token, fields map[string]gradedRustFlow, contracts map[string]bool, declarations map[int]gradedOutputDeclaration, calls map[int]call, locals map[string]gradedRustFlow, scopes *[]map[string]gradedRustFlow, i int, t token, flow *unconditionalQueries) (destroyed, moved bool) {
	if t.text == "{" {
	...
	return gradedRustPointerCall(body, fields, locals, c, flow)
}

func gradedRustPointerCall(body []token, fields map[string]gradedRustFlow, locals map[string]gradedRustFlow, c call, flow *unconditionalQueries) (destroyed, moved bool) {
	if strings.HasSuffix(c.name, ".drop_in_place") && len(c.actuals) == 1 && flow.unconditional(body, c.position) {
