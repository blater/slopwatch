...
func referenceRustFunctions(tokens []token) []rustFunction {
	result := make([]rustFunction, 0, 8)
	impls := referenceRustImplRanges(tokens)
	types := referenceRustNamedRanges(tokens, "struct")
	for i := 0; i < len(tokens); i++ {
		...
		if info.impl != nil {
			info.traitPub = referenceRustTraitPublic(tokens, info.impl.trait)
		}
...
func referenceRustImplRanges(tokens []token) []rustImplRange {
...
func referenceRustNamedRanges(tokens []token, keyword string) []rustRange {
...
func referenceRustTraitPublic(tokens []token, name string) bool {
	if name == "" {
		return false
	}
	for _, d := range referenceRustNamedRanges(tokens, "trait") {
		if d.name == name && d.public {
			return true
		}
	}
	return false
