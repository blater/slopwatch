...
func TestRunRejectsSuccessfulRawAndJSONEscapedTokenEcho(t *testing.T) {
	...
	for _, output := range []string{
		...
		service := &Service{client: ghClient{executable: "/usr/bin/gh", directory: "/safe", getenv: lookup, runner: runnerFunc(func(context.Context, isolation.Request) (isolation.Result, error) {
			return isolation.Result{ExitCode: 0, Stdout: []byte(output)}, nil
		})}}
		result, err := service.client.run(withCommandOutput(t.Context(), testCommandOutputBytes), "pr", "list")
...
func TestCreateRejectsKnownTokenBeforeGitHubCLIExecution(t *testing.T) {
	...
	called := false
	service := &Service{client: ghClient{executable: "/usr/bin/gh", directory: "/safe", getenv: func(key string) string {
		if key == "GH_TOKEN" {
			return token
		}
		return ""
...
func TestCreateAndLookupRejectHostilePullRequestURLs(t *testing.T) {
	...
	for _, hostile := range []string{
		...
		t.Run(hostile, func(t *testing.T) {
			...
			service := &Service{client: ghClient{executable: "/usr/bin/gh", directory: "/safe", getenv: func(string) string { return "" }, runner: runner}}
			result, err := service.Create(t.Context(), request)
			...
			service.client.runner = lookupRunner
			if result, found, err := lookup(t.Context(), service.client, request, 0); err == nil || found || result.URL != "" {
...
func TestReconcileRejectsUnverifiedSavedIdentity(t *testing.T) {
	service := &Service{client: ghClient{executable: "/usr/bin/gh", directory: "/safe", getenv: func(string) string { return "" }, runner: &sequenceRunner{results: []isolation.Result{{ExitCode: 0, Stdout: []byte("[]")}}}}}
...
func TestCreatePersistsExactNumberAndRejectsCrossNumberReconciliation(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			service := &Service{client: ghClient{executable: "/usr/bin/gh", directory: "/safe", getenv: func(string) string { return "" }, runner: &sequenceRunner{results: []isolation.Result{
				{ExitCode: 0, Stdout: []byte("[]")},
...
func TestCreateFailureIsAmbiguousAndReconcileBindsSavedNumber(t *testing.T) {
	request := publisher.Request{HostRepository: "owner/repo", BaseBranch: "main", HeadBranch: "fix/test", Commit: "abc", Draft: true, CommandOutputBytes: testCommandOutputBytes}
	calls := 0
	service := &Service{client: ghClient{executable: "/usr/bin/gh", directory: "/safe", getenv: func(string) string { return "" }, runner: runnerFunc(func(_ context.Context, launched isolation.Request) (isolation.Result, error) {
		calls++
		if calls == 1 {
	...
	service.client.runner = &sequenceRunner{results: []isolation.Result{{ExitCode: 0, Stdout: []byte(
...
func TestLookupRequiresExactUniqueOpenDraftSameRepositoryMatch(t *testing.T) {
	...
	for _, records := range []string{
		...
		service := &Service{client: ghClient{executable: "/usr/bin/gh", directory: "/safe", getenv: func(string) string { return "" }, runner: &sequenceRunner{results: []isolation.Result{{ExitCode: 0, Stdout: []byte(records)}}}}}
		if result, found, err := lookup(t.Context(), service.client, request, 0); err == nil || found || !result.Ambiguous {
...
func TestLookupExpandsResultWindowWithoutCompiledPRCountCap(t *testing.T) {
	...
	service := &Service{client: ghClient{executable: "/usr/bin/gh", directory: "/safe", getenv: func(string) string { return "" }, runner: runner}}
	result, found, err := lookup(t.Context(), service.client, request, 0)
...
func TestRunRedactsForwardedTokensFromStderrAndRunnerErrors(t *testing.T) {
	...
	for _, runner := range []runnerFunc{
		...
		service := &Service{client: ghClient{executable: "/usr/bin/gh", directory: "/safe", getenv: lookup, runner: runner}}
		_, err := service.client.run(withCommandOutput(t.Context(), testCommandOutputBytes), "auth", "status")
...
func TestRunPinsExecutableWorkingDirectoryChildPathAndHost(t *testing.T) {
	...
	service := &Service{client: ghClient{executable: "/trusted/bin/gh", directory: "/private/publisher", getenv: func(key string) string {
		switch key {
		case "PATH":
	...
	if _, err := service.client.run(withCommandOutput(t.Context(), testCommandOutputBytes), "auth", "status", "--hostname", "github.com"); err != nil {
		t.Fatal(err)
	}
