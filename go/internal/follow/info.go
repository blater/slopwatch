...
var metricInformation = []metricInfo{
	{key: "score", label: "SCORE", short: "overall score", description: "Each routine contributes its strongest enabled, weighted control-flow signal: cognitive complexity, cyclomatic complexity, NPath, or nesting. These routine contributions and other design contributions form SCORE. Raw metric values remain visible; supporting signals are not additional charges. Lower is better."},
	...
	{key: "nesting", label: "NEST", short: "deep nesting", description: "NEST counts control-flow branches nested beyond the configured depth. When enabled, its weighted severity competes with the other control-flow signals for each routine; it does not add a second charge for nesting already reflected in COG. It is disabled in the dashboard by default. Lower is better."},
