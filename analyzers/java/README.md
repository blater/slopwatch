...
reported even when they are below the active Slopslap scoring threshold.
...
Java classpath and run PMD with `slopslap-bridge-ruleset.xml`. Analyzed
projects do not receive this dependency.

Each `SlopslapMetrics` violation message begins with
`SLOPSLAP_METRIC_V1` and contains a stable pipe-delimited observation. The
