  ...
  if (owner.sources.length === 0) {
    ...
    try {
            ...
            diagnostics: [
              {
                ...
                attributes: { tsconfigs: [...discovered].sort().map((item) => posixPath(path.relative(owner.workspace, item))) }
              }
            ],
      ...
      if (tsconfigPath !== undefined) {
        ...
        if (parsed.errors.length > 0) return owner.configFailure(parsed.errors, tsconfigPath);
    ...
    const requested = new Map(
      owner.sources.map((item) => [owner.canonicalKey(item.absolutePath), item.absolutePath])
    );
    ...
    if (compilerDiagnostics.length > 0) {
      for (const item of compilerDiagnostics.slice(0, 50)) {
        ...
        const diagnosticPath = sourcePath === undefined ? undefined : owner.relativeIfInside(sourcePath);
        const attributes =
