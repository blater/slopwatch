...
test("CLI accepts a one-record NDJSON request and emits one terminal record last", () => {
  ...
  try {
    ...
    const request = {
      ...
      workspace,
      units: [{ unit_id: "unit", language: "typescript", source_paths: ["entry.ts"] }],
      components: [{ component_id: "npath_complexity", definition_version: "pmd-v1" }],
    ...
    assert.ok(records.every((record) => record.invocation_id === "protocol-test"));
  } finally {
